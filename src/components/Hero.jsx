...
// The four sub-steps that make up one challenge (Reading / Listening). Finishing
// all four advances the learner one sub-level (e.g. B1 → B1.1).
const SUB_LEVEL_STOPS = [
  { id: 'comprehension', label: 'Compréhension', x: 84 },
  { id: 'vocabulary',    label: 'Vocabulaire',   x: 148 },
  { id: 'grammar',       label: 'Grammaire',     x: 212 },
  { id: 'conjugation',   label: 'Conjugaison',   x: 276 },
];

// Shown in place of the exercise content once a sub-tab is fully validated:
// a B1 → B1.1 progress line with the four sub-steps, plus a Continue button.
function SubLevelProgressLine({ title, level, doneSet, justCompleted, allDone, onContinue, continueLabel }) {
  const WINE = '#8B1E2D';
  const next = `${level}.1`;
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full px-3 py-6 text-center">
      <p className="font-display text-[20px] text-navy leading-none mb-1">{title}</p>
      <p className="text-[10px] font-mono tracking-[0.2em] uppercase text-wine/60 mb-5">
        {level} → {next}
      </p>
      <svg viewBox="0 0 360 58" width="100%" className="max-w-[360px]" role="group"
        aria-label={`Progression de ${level} à ${next}`}>
        {seg(20, 84, done.has('comprehension'), 's0')}
        {seg(84, 148, done.has('vocabulary'), 's1')}
        {seg(148, 212, done.has('grammar'), 's2')}
        {seg(212, 276, done.has('conjugation'), 's3')}
        {seg(276, 340, allDone, 's4')}
        <path d="M331 19 L341 24 L331 29 Z" fill={WINE} opacity={allDone ? 1 : 0.55} />

        <circle cx="20" cy="24" r="12" fill={WINE} />
        <text x="20" y="27.5" textAnchor="middle" fill="#F6F1E8"
          fontFamily="'SF Mono',monospace" fontSize="10" fontWeight="700">{level}</text>
        ...
        <text x="340" y="27.5" textAnchor="middle" fill={allDone ? '#F6F1E8' : WINE}
          fontFamily="'SF Mono',monospace" fontSize="8.5" fontWeight="700">{next}</text>

        {SUB_LEVEL_STOPS.map((s) => {
          const isDone = done.has(s.id);
          return (
            <g key={s.id}>
              <circle cx={s.x} cy="24" r="7" fill={isDone ? WINE : '#fff'} stroke={WINE} strokeWidth="1.6" />
              {isDone && (
                <path d={`M${s.x - 3.2} 24 L${s.x - 0.8} 26.6 L${s.x + 3.6} 21.2`}
                  stroke="#F6F1E8" strokeWidth="1.7" fill="none" strokeLinecap="round" strokeLinejoin="round" />
              )}
              <text x={s.x} y="45" textAnchor="middle" fill={isDone ? WINE : '#1A2340'}
                fillOpacity={isDone ? 1 : 0.5} fontFamily="Georgia,serif" fontStyle="italic" fontSize="8.5">{s.label}</text>
            </g>
          );
        })}
      </svg>
      <button type="button" onClick={onContinue}
        className="mt-6 inline-flex items-center gap-2 rounded-full bg-wine hover:bg-wine2 text-ivory px-5 py-2 text-[13px] font-display transition-colors">
        {continueLabel}
        ...
        {options.map((opt, oi) => {
          ...
          return (
            <button key={oi} type="button" disabled={answered}
              onClick={() => { if (!answered) { setAnsweredIdx(oi); firePointsDelta(oi === correctIdx ? 3 : -1); onAnswered?.(oi === correctIdx); } }}
        ...
        <>
          ...
          {title && (
            <div className="relative mb-5 shrink-0 mr-3 px-4 py-3 border-l-4 border-navy bg-navy/5" style={{ borderRadius: '0 4px 4px 0' }}>
              ...
              <p className="text-[9px] font-mono tracking-[0.18em] uppercase text-wine/60 truncate mb-1.5 pr-12">
        ...
        <>
          ...
          {title && (
            <div className="relative mb-2 shrink-0 px-3 py-2 border-l-4 border-navy bg-navy/5" style={{ borderRadius: '0 4px 4px 0' }}>
              ...
              <p className="text-[9px] font-mono tracking-[0.18em] uppercase text-wine/60 truncate mb-1 pr-12">
