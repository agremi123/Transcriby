          ...
          <div className="ml-7 flex overflow-x-auto border-t border-line/50">
            {[
              ...
              <button key={t.id} type="button" onClick={() => setActiveTab(t.id)}
                className={`text-[10px] tracking-wide uppercase px-2.5 py-3 border-b-2 transition-colors whitespace-nowrap shrink-0 ${activeTab === t.id ? 'border-wine text-wine' : 'border-transparent text-navy/70 hover:text-navy'}`}>
                {t.label}
              </button>
            ))}
          </div>
