...
async function generateListeningBundle(apiKey, level = 'B1') {
  ...
  const [{ startSec, endSec }, qParsed, vParsed, gParsed, cParsed] = await Promise.all([
    ...
    claudeJSON({ apiKey, maxTokens: 2000, system: `French grammar teacher. Exactly 2 grammar structures from transcript for ${level} learner, each with exactly 6 fill-in-the-blank exercises (one "___" per sentence; hint = base form of answer). Raw JSON: {"grammar":[{"point":"...","example":"...","explanation":"...","tip":"...","exercises":[{"sentence":"...___...","answer":"...","hint":"..."}]}]}`, user: previewTranscript }),
